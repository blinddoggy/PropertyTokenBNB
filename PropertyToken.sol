    ...
    ERC721Burnable
{
    ...
    function approveMaster(uint256 amount,address PropertyMaster) public {
        require(msg.sender == owner(), "Only the owner can approve the master");
        require(usdt.approve(address(PropertyMaster), amount), "Approval failed");
    }

    function transferFromMaster(address recipient, uint256 amount) public {
        require(msg.sender == owner(), "Only the owner can transfer from the master");
        require(usdt.transferFrom(address(this), recipient, amount), "Transfer failed");
    }
